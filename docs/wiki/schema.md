...
| `installer` | Provisioning: entry point and profiles, persisted state, platform detection, package lists, package-manager ladder, runtime versions, config seeding | install.sh, core.sh, packages, mise, profiles, light, dev, idempotence, seeding, symlink |
| `shell-runtime` | Loader order, per-operating-system scoping, `zz_` tail-init contract, `#` opt-out convention, startup cost | .zshrc, modules, preload_configs, load order, zz_, compinit, completions, zoxide |
